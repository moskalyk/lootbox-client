require(['lib/three', 'lib/tween', 'dungeon', 'relativeDir', 'constants'], function(THREE, TWEEN, Dungeon, RelativeDir, Const) {
    ...
    var dungeonElement = document.getElementById('dungeon');

    // If the element exists, replace it with the renderer's DOM element
    if (dungeonElement) {
        document.body.replaceChild(renderer.domElement, dungeonElement);
    } else {
        console.error('Element with ID "dungeon" not found.');
    }
